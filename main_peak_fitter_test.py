...
input_folder =r"/Users/benkupernk/Documents/GitHub/XRD-Fitting/test_files"# os.path.join(general_input_folder, sample_name, 'integration')

general_output_folder = r'/Users/benkupernk/Documents/GitHub'
...
# Graphite/LiC12 only
q_range_dict = {'Graphite_LiC12':[1.75, 1.9, 500, 0.005, 5]} #Stage 2, 3, 4

# LiC6 only
#q_range_dict = {'LiC6':[1.6, 1.75, 15, 0.1, 100]} #Stage 1

# nmc peaks only
#q_range_dict = {'NMC_003':[1.25, 1.36, 20, 0.1, 30]}
#q_range_dict = {'NMC_other':[2.55, 2.74, 5000, 0.1, 30]}
...
#q_range_dict = {'Cu_111':[2.95,3.06, 5000, 0.2, 500]}
...
for element_name, element_values in q_range_dict.items(): # for each peak defined in q_rage_dict
    ...
    # does the same thing as this || general_peak = Graphite_LiC12('Graphite_LiC12', 1.75, 1.9, 500, 0.005, 5) 
    # which is to say it instancates the class for the element were looking for.
    general_peak = eval(element_name)(element_name, *element_values)
    ...
    print("Finding ", element_name, " peaks! Hold on to your socks!")
    print("qmax is " , general_peak.q_min, ", qmin is ", general_peak.q_max)
    ...
    for i in range(last_i, len(list_of_files)):
        ...
        #i = 150
        # 
        i_list = [2]# [26, 28, 32, 66, 68,70,  72, 74, 75, 78, 80]
        if True:#i in i_list:
            ...
            if 'mean_q' in list_of_files[i]:
                ...
                if x >= x_min and x <= x_max:
                    if y >= y_min and y <= y_max:
                        ...
                        savePath = pf.save_fits(plot_folder, get_integrals, element_name, list_of_files, i, sample_name)
                       ...
                       # df_integrals_temp['Model Path'].loc[max_row + 1] = savePath
                        ...
                        file_name = str(get_integrals[0] + '_' + element_name + '.csv')
