...
import Specific_peak_fitting_functions_test as lpf
import numpy as np
import pandas as pd
import os
...
def fit_data_v2(sliced_q, sliced_I, q_max, q_min, num_of_centers, 
                      sig, amp, chisqu_fit_value, Li_q_max, Li_q_min,
                      x_motor, y_motor, peak_name, plot, general_peak):
...
def fit_data(sliced_q, sliced_I, q_max, q_min, num_of_centers, sig, amp, 
             chisqu_fit_value, Li_q_max, Li_q_min, x_motor, y_motor, 
             peak_name, plot, general_peak, call_user_fit):
    # chisqr = 1000000000
    # initial_fit = False
    # fwhm_too_big = False
    # good_model = False
    
    # while good_model == False:
     peak_vals = general_peak.intial_values(sliced_I, sliced_q)   
     specific_peak = general_peak.find_best_sub_peak(peak_vals, sliced_q, sliced_I)
     peak_vals = specific_peak.secondary_values(peak_vals, sliced_I, sliced_q) 
     # Get the peak center ('peak') and the prominences of the peak
     
     print('Peak class chosen = ', specific_peak.name)
     model_list = specific_peak.make_all_modles(peak_vals)
    ...
    # if you want you can change to -1!! might be good if the thing happens a lot
     ...
     plot_peaks(best_model, sliced_q, sliced_I, x_motor, y_motor, peak_name, plot)
     good_model = specific_peak.check_fit(best_model, sliced_q, sliced_I, peak_vals)
     
     
     # chisqr = best_model.chisqr    
     
     
     if good_model == False and call_user_fit == True:
         best_model = ufo.user_model(best_model, sliced_q, sliced_I, sig, amp, q_max, q_min, chisqu_fit_value, x_motor, y_motor, peak_name, plot)
    ...
    #     results_sorted = sorted(model_result_list, key=lambda model: model.chisqr)
    #     best_model = results_sorted[0]
    #     good_model = specific_peak.check_fit(best_model, sliced_q, sliced_I, sig, amp, q_max, q_min, chisqu_fit_value, x_motor, y_motor, peak_name, plot)
...
def master_function(i, read_sample_file, num_of_centers,  data_path, q_min, q_max,
                    sample_name, sig, amp, chisqu_fit_value, peak_name, Li_q_max,
                    Li_q_min, plot, general_input_folder, general_peak, call_user_fit):
    
    # Make a dataframe of the entire XRD pattern
    df = make_dataframe(read_sample_file, data_path)
    df.to_csv(r'C:\Users\Elizabeth Allan-Cole\Desktop\XRD Data Processing\NSLS-II Winter 2023\Processing\Initial_fit\Class-testing\Peak\data_%s.csv' % i )
    ...
    best_model = fit_data(sliced_q, sliced_I, q_max, q_min, num_of_centers, 
                          sig, amp, chisqu_fit_value, Li_q_max, Li_q_min,
                          x_motor, y_motor, peak_name, plot, general_peak, call_user_fit)
